...
public class WorkoutFlowIntegrationTests
{
    ...
    public async Task ProgressWeek_ToWeek7_IdentifiesAsDeloadWeek()
    {
        ...
        // Act - Check that we're now at week 7 (deload week)
    ...
    public async Task Full21WeekCycle_AllProgressionTypes_VerifiesProgressionAndHevySync()
    {
        ...
        for (int week = 1; week <= 21; week++)
        {
            ...
            if (week < 21)
            {
                ...
                // Assert bilateral RepsPerSet (Lat Pulldown) sets & weight
                ...
                // Assert unilateral RepsPerSet (Single Leg Press) sets & weight
    ...
    /// <summary>
    /// Asserts the WeekPlan endpoint returns correct planned sets for Hevy sync.
    /// Verifies set count, weight, reps, and AMRAP flags for tracked exercises.
    /// </summary>
    ...
    public async Task CompleteFullWeek_ThenProgress_WorksCorrectly()
    {
        ...
        // Assert - The last day completion should have triggered week progression
        lastDayResult.Should().NotBeNull();
