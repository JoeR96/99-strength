...
public sealed class RemoveExerciseCommandHandler : IRequestHandler<RemoveExerciseCommand, Result<bool>>
{
    ...
    public RemoveExerciseCommandHandler(
    ...
    public async Task<Result<bool>> Handle(RemoveExerciseCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            workout.RemoveExercise(new ExerciseId(request.ExerciseId));
        ...
        {
            return Result.Failure<bool>($"Failed to remove exercise: {ex.Message}");
        }
    }
}
